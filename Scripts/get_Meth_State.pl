...
if(scalar (@ARGV) != 4)
{
	print "use for: get the Meth with respect to the center of state if its within distance ARGV4\n";
...
#			{
				...
				#if (($key - $ARGV[3]) <= $center  and ( $center <= ( $key + $ARGV[3]) ))  
				#if (($key - $ARGV[3]) < $parts[1]  and ( $parts[2] <= ( $key + $ARGV[3])) )  
				{
					$dist = $key - $center;
					if ($dist == 0 or ($key >= $parts[1] and $key <= $parts[2] ))
					{
						print 	OUTPUT "$parts[3],0,$list{$parts[0]}{$key}\n"; # $parts[1],$parts[2],$parts[3],$parts[4]\n";
					}
					else #if ($dist > 0)
					{
						print 	OUTPUT "$parts[3],$dist,$list{$parts[0]}{$key}\n"; # $parts[1],$parts[2],$parts[3],$parts[4]\n";
					}
					#else
				#	{
