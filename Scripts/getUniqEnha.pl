...
if(scalar (@ARGV) != 3)
{
	print "use for: get the uniq enhancer that close to the gene TSS\n";
	print "need three arguments\n";
	print "first start Enha\n";
	print "Second end enha\n";
...
while(<List>)
{
	...
	@parts = split(/[,\s]+/, $line);
		...
		# accordinaly 
		if ($Enh{$parts[0]}{$parts[1]}{$parts[2]}{$parts[]} > $ARGV[3]) # set up the upper limit for positive and negative signe
		{
