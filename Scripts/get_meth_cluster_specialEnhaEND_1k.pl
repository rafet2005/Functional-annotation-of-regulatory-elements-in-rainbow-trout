...
#{
	if ($line !~ /^#/)
	{
		...
		foreach my $key(keys %{$list{$parts[0]}})
		{
			...
			{
				if (($key - $ARGV[3]) < $parts[2] and ($key + $limit{$parts[0]}{$key}    > $parts[2] ))  # end of enhancer 
                #	if (($key - $ARGV[3]) < $parts[1] and ($key + $limit{$parts[0]}{$key}    > $parts[1] ))  # start of enhancet
				{
					#	print "Positive,$parts[1],$parts[0],$key,$ExonStart{$parts[0]}{$key},$ExonEnd{$parts[0]}{$key}\n";
					$line.=",". $list{$parts[0]}{$key}; #  add gene name;
					if (($key ) <= $parts[2] and ($end{$parts[0]}{$key}  >=  $parts[2] ))  
					...
					$line.=",".$key.",". $end{$parts[0]}{$key}.",".$signe{$parts[0]}{$key}; # $parts[2],$parts[2],$parts[3],$parts[4]\n";
					if($parts[2] > $ExonStart{$parts[0]}{$key}  and $parts[2] < $ExonEnd{$parts[0]}{$key} )
					{
						$line.= "," ."FirstExon"; # $parts[2],$parts[2],$parts[3],$parts[4]\n";
					}
                    else
                    {
						$line.= ",NA"; # $parts[2],$parts[2],$parts[3],$parts[4]\n";
			...
			{
				if (  ($end{$parts[0]}{$key} - $Nelimit{$parts[0]}{$key})  <= $parts[2] and (($end{$parts[0]}{$key} + $ARGV[3] )  >= $parts[2] ))  
				{
					#	print "Negative,$parts[1],$parts[0],$key,$ExonStart{$parts[0]}{$key},$ExonEnd{$parts[0]}{$key}\n";
					$line.=",". $list{$parts[0]}{$key}; # $parts[2],$parts[2],$parts[3],$parts[4]\n";
					if (($key ) <= $parts[2] and ($end{$parts[0]}{$key}  >=  $parts[2] ))  
					{
						$line.=",". "Body,NA,NA,NA,NA,NA"; # $parts[2],$parts[2],$parts[3],$parts[4]\n";
					}
					else
					{
						$line.=",". "NA"; # $parts[2],$parts[2],$parts[3],$parts[4]\n";
						if (($end{$parts[0]}{$key} + 2000 >= $parts[2]) and ($end{$parts[0]}{$key}  <= $parts[2] ))  
						{
							$line.= "," ."1K,2K,3K,5K,10K"; # $parts[2],$parts[2],$parts[3],$parts[4]\n";
							$countP++;
						}
						elsif (($end{$parts[0]}{$key} + 2000 >= $parts[2]) and ($end{$parts[0]}{$key}  <= $parts[2] ))  
						{
							$line.= "," ."NA,2K,3K,5K,10K"; # $parts[2],$parts[2],$parts[3],$parts[4]\n";
						}
						elsif (($end{$parts[0]}{$key} + 3000) >= $parts[2] and ($end{$parts[0]}{$key}  <= $parts[2] ))  
						{
							$line.= "," ."NA,NA,3K,5K,10K"; # $parts[2],$parts[2],$parts[3],$parts[4]\n";
						}
						elsif (($end{$parts[0]}{$key} + 5000) >= $parts[2] and ($end{$parts[0]}{$key}  <= $parts[2] ))  
						{
							$line.= "," ."NA,NA,NA,5K,10K"; # $parts[2],$parts[2],$parts[3],$parts[4]\n";
						}
						elsif (($end{$parts[0]}{$key} + $ARGV[3]) >= $parts[2] and ($end{$parts[0]}{$key}  <= $parts[2] ))  
						{
							$line.= "," ."NA,NA,NA,NA,10K"; # $parts[2],$parts[2],$parts[3],$parts[4]\n";
						}
						else
						{
							$line.=",". "NA,NA,NA,NA,NA,NA"; # $parts[2],$parts[2],$parts[3],$parts[4]\n";
						}
					}
					$line.=",".$key.",". $end{$parts[0]}{$key}.",".$signe{$parts[0]}{$key}; # $parts[2],$parts[2],$parts[3],$parts[4]\n";
					if($parts[2] > $ExonStart{$parts[0]}{$key}  and $parts[2] < $ExonEnd{$parts[0]}{$key} )
