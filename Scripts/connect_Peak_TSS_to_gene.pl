...
if(scalar (@ARGV) != 3)
{
	print "use for:to add sign from gene file to  TSS Peak file with distance \n";
	print "need three arguments\n";
	print "first TSS_gene_new_sorted.csv\n";
	print "Second TSS_Peak file\n";
...
#	$list{$parts[0]}{$center} = $line;
	$list{$parts[0]}{$parts[1]} =$parts[5].','.$parts[6].','.$parts[7]; # $line; # reading the CPG file first
...
print 	OUTPUT "chr,TES,Sumit,dist,genStart,genEnd,sign\n";
while(<INPUT>)
{
	chomp;
	$line = $_;
...
#			{
				...
				if ($key == $parts[1])# and $end == $parts[4])  or ($end == $parts[4] and ($key + 1) == $parts[3]) )  
				#if (($key - $ARGV[3]) < $parts[1]  and ( $parts[2] < ( $key + $ARGV[3]) ))  
				{
						print 	OUTPUT "$line,$list{$parts[0]}{$key}\n";#\t$parts[2]\t$parts[3]\t$parts[4]\t$parts[6]\n"; # $parts[1],$parts[2],$parts[3],$parts[4]\n";
						last;
