...
#### Trim low-quality base and adpater using trimgalor and fastqc

``` bash
# check QC withn same directroy of the fastq
# -t number of threads
fastqc -t 6 *.fq.gz
multiqc .
firefox multiqc_report.html

# or create a folder such as trim_fastq
mkdir trim
cd trim
...
# --fastqc will run teh fastqc and remove any low quality reads
```
...
# for pair end reads adding the 2>> tmp_file will save the stat of the mapping
 ...
 # using Samtools sort bam file and create index file
 ...
 # Use picard tools to marke duplicate                                                                      
for i in *filtered_sorted.bam; do out=${i/orted/orted_PCRDup};  java -jar ~/softwar/       Picard/picard.jar MarkDuplicates I=$i O=$out  M=Aligned_Sorted_PCRD.txt AS=true ; done
```
...
# the following parameters were used for macs2
  ...
  --keep-dup, '3': this is an arbitary number and we selcet to keep three  
 ...
 # We calculated peak numbers for each mark as follow:
...
``` bash
# Analysis follows the same procedures in the chipSeq and used broad peaks
...
# QC and trim using cut adapter through Trimglore and used fastqc for quality control
 ...
 #Bismark_genome_preparation ../genome/ the folder of genome has a fast file and include bowtie index files too
 #genome had to be prpaired before using
...
#further analysis using local script
# append to the file info about CPG location within the premoter and TSS               
get_meth_cluster_1k.pl to get location within TSS

#get island info from isalnd files from NCBI and annotate CpG file
```
...
``` bash
#chromHMM analysis
 # 2. prepare the genome files as follow
 #find the chromosom length and store the file to the directory chromHMM directory
 samtools faidx refrenceGenome.fa
 cut -f1,2 refrenceGenome.fai > TroutArrly_chrom_lenth
  
 #3. convert gtf or gff to genePred and bigGenePred.txt file of ucsc broweser use the next two steps
 ...
 #4. add the trout genome to the chromHmm directory by runining the following code in the ~/path/ChromHMM/ANCHORFILES directory
...
for i in *sorted_PCRDup.bam; do out=${i/\.bam/\.bed} ; bamToBed -i $i > $out ; done
 ...
 #5. prepare the cellmarkfiletable.txt file such as
 #use all availabe tissue in our case we sex chipseq and 3 atac seq 
...
#6. convert bed file to binarize as one of these command
...
#7. run the model using different model and determin the one based on genomic features
...
``` bash
# the following analysis used to create data for several plots
 #for emmission coverage use the missions_xx.txt file
 #convert it to csv and clean the header and reorder it using awk script in vim
 % !awk 'BEGIN {OFS=","}; {print $1, $6, $3, $4, $5, $2}'
  
 # #### calculate Cov average and std
 #create overlap list using linux command
 ...
 #after cleaning the file and convert it to csv remove top and bottom lines
...
# and plot it as heatmap

 
## retrive the indvidual stat for all tissue combind into one file OLD APPROCHE
##./parse_chrohmm_chr_start_any.pl file_list
 
#combin state for down analysis not use for enrichment use the python and the actule       result 
...
# this will addd the stat to the file which needed later on for the cluster file
...
# Arrly_annotaion_file_Enrichmnet_sorted file that has all the features as bed file such as
